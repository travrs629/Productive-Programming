...
|---|---|---|
|Requests|HTTP library|Provide a simple yet useful API for interacting with     `HTTP` operations such as `GET`, `POST`, etc.<br>1. Can send request to the HTTP server, and `GET` response back in the form of `HTML` or `JSON` response. <br>2. Can send `POST` requests to modify or add some content.|
|BeautifulSoup||
|Scrapy||
|Selenium||
|Urllib3||
